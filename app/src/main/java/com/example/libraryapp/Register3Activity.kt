...
class Register3Activity : AppCompatActivity() {
    ...
    private val reviewsApi = RetrofitClientInstance.bookApi
    ...
    private fun postKeywordsToServer(selectedKeywords: List<String>) {
        CoroutineScope(Dispatchers.IO).launch {
            try {
                val response = reviewsApi.postKeywords(selectedKeywords)
                if (response.isSuccessful) {
                    runOnUiThread {
                        val intent = Intent(this@Register3Activity, HomeActivity::class.java)
                        intent.putStringArrayListExtra("key", ArrayList(selectedKeywords.toList())) // ArrayList로 변환하여 전달
                    ...
                    runOnUiThread {
                        Toast.makeText(this@Register3Activity, "$errorMessage", Toast.LENGTH_SHORT).show()
                    }
                }
            } catch (e: Exception) {
                runOnUiThread {
